...
void loop()
{
	...
	{
		printf("Init: Cannot connect to uROS agent. IP=%d.%d.%d.%d, port=%s\n", agent_ip[0], agent_ip[1], agent_ip[2], agent_ip[3], agent_port);
		M5.Lcd.printf("Cannot connect to uROS agent IP=%d.%d.%d.%d, port=%s\n", agent_ip[0], agent_ip[1], agent_ip[2], agent_ip[3], agent_port);
		vTaskDelete(NULL);
	}
...
* Date: 2021/7/30
*******************************************************************************
  PC and M5Core2 can only be used on the same wifi.
电脑和M5Core2需在同一wifi下才可使用 When the OTA is ready, restart the Arduino
...
void setup() {
    ...
    wifiMulti.addAP("eurobin_iot",
                    "123456789");  // Storage wifi configuration information 1.
                                  // 存储wifi配置信息1
    //wifiMulti.addAP("RLAN_INRIA_2G", "R0b0t1cs");
